...
export const useAuthStore = defineStore('auth', () => {
  ...
  const currentUserName = computed(() => userContext.value?.fullName || '未登入');
  ...
  function hasPermission(permission: string): boolean {
    ...
    return !!userContext.value.permissions[permission as keyof typeof userContext.value.permissions];
  }

  /** Check reveal authority for a field */
  function canReveal(field: string): boolean {
    if (!userContext.value) return false;
    return !!userContext.value.revealAuthority[field as keyof typeof userContext.value.revealAuthority];
  }
  ...
  async function switchUser(userId: string): Promise<void> {
    isLoading.value = true;
    try {
      const response = await $fetch<AuthContextResponse & { success: boolean }>('/api/auth/switch-user', {
        method: 'POST',
        body: { userId },
      });
      userContext.value = response.user;
